...
export class CanonicalStore {
    ...
    `).run(project.id, project.name, project.repoRoot, project.repoFullName, project.workflowConfigPath ?? null, JSON.stringify(project.allowedExecutors));
        return project;
    }
    listProjects() {
        return this.db.prepare("SELECT * FROM projects ORDER BY name").all().map((row) => ({
            id: row.id,
            name: row.name,
    ...
    upsertWorkItem(workItem) {
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        url = excluded.url
    `).run(workItem.id, workItem.projectId, workItem.kind, workItem.externalId, workItem.title, workItem.state, JSON.stringify(workItem.labels), workItem.url);
        return workItem;
    }
    getWorkItemByExternalId(projectId, externalId) {
        const row = this.db.prepare("SELECT * FROM work_items WHERE project_id = ? AND external_id = ? LIMIT 1").get(projectId, externalId);
        return row ? {
            id: row.id,
    ...
    listWorkItems(projectId) {
        return this.db.prepare("SELECT * FROM work_items WHERE project_id = ? ORDER BY external_id DESC").all(projectId).map((row) => ({
            id: row.id,
            projectId: row.project_id,
            kind: row.kind,
            externalId: row.external_id,
    ...
    createWorkflowInstance(input) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(workflow.id, workflow.projectId, workflow.workItemId, workflow.stage, workflow.status, workflow.repairRound, workflow.createdAt, workflow.updatedAt, input.branch);
        return workflow;
    }
    ...
    getWorkflowInstance(id) {
        const row = this.db.prepare("SELECT * FROM workflow_instances WHERE id = ?").get(id);
        return row ? {
            id: row.id,
            projectId: row.project_id,
    ...
    getWorkflowBranch(id) {
        const row = this.db.prepare("SELECT branch FROM workflow_instances WHERE id = ?").get(id);
        if (!row)
            throw new Error(`Workflow ${id} not found`);
        return row.branch;
    }
    listWorkflowInstances() {
        return this.db.prepare("SELECT * FROM workflow_instances ORDER BY updated_at DESC").all().map((row) => ({
            id: row.id,
            projectId: row.project_id,
            workItemId: row.work_item_id,
            stage: row.stage,
    ...
    listTasks(workflowInstanceId) {
        return this.db.prepare("SELECT * FROM tasks WHERE workflow_instance_id = ? ORDER BY rowid").all(workflowInstanceId).map((row) => ({
            id: row.id,
            workflowInstanceId: row.workflow_instance_id,
            type: row.type,
            status: row.status,
    ...
    listAttempts(taskId) {
        return this.db.prepare("SELECT * FROM execution_attempts WHERE task_id = ? ORDER BY started_at").all(taskId).map((row) => ({
            id: row.id,
            taskId: row.task_id,
            executorId: row.executor_id,
            runnerId: row.runner_id,
    ...
    getPendingApproval(workflowInstanceId) {
        const row = this.db.prepare("SELECT * FROM approvals WHERE workflow_instance_id = ? AND status = 'pending' ORDER BY rowid DESC LIMIT 1").get(workflowInstanceId);
        return row ? {
            id: row.id,
            workflowInstanceId: row.workflow_instance_id,
            stage: row.stage,
    ...
    recordEvent(taskId, event) {
        this.db.prepare("INSERT INTO task_events (task_id, event_json, created_at) VALUES (?, ?, ?)").run(taskId, JSON.stringify(event), now());
    }
    listEvents(taskId) {
        return this.db.prepare("SELECT * FROM task_events WHERE task_id = ? ORDER BY id").all(taskId).map((row) => ({
            id: row.id,
            taskId: row.task_id,
    ...
    recordArtifacts(taskId, artifacts) {
    ...
    listArtifacts(taskId) {
        return this.db.prepare("SELECT * FROM task_artifacts WHERE task_id = ? ORDER BY created_at").all(taskId).map((row) => ({
            kind: row.kind,
            path: row.path,
    ...
    getWorkflowSnapshot(id) {
        ...
        const approvals = this.db.prepare("SELECT * FROM approvals WHERE workflow_instance_id = ? ORDER BY rowid").all(id).map((row) => ({
            id: row.id,
            workflowInstanceId: row.workflow_instance_id,
            stage: row.stage,
            status: row.status,
