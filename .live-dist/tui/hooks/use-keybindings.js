...
export function useKeybindings(actions, screen, inputMode) {
    ...
    useInput((input, key) => {
        ...
        if (screen === "run") {
            if (key.tab && !key.shift)
                actions.onNextDetailTab();
            if (key.tab && key.shift)
                actions.onPrevDetailTab();
        }
        else {
            if (key.tab && !key.shift)
                actions.onNextPane();
        ...
        // Log modes
        if (input === "S")
            actions.onSetLogMode("structured");
        if (input === "L")
            actions.onSetLogMode("raw");
        // Run actions
        if (input === "a" || input === "A")
            actions.onApprove();
        if (input === "c" || input === "C")
        ...
        if (input === "q" || input === "Q")
            actions.onQuit();
        if (input === "i" || input === "I")
            actions.onEnterInput();
        if (input === "o" || input === "O")
            actions.onOpenExternal();
        if (input === "v" || input === "V")
            actions.onApprovalsView();
        if (input === "f" || input === "F")
            actions.onToggleDiff();
        if (input === "p" || input === "P")
            actions.onPause();
        if (input === "t" || input === "T")
            actions.onTakeOver();
        if (input === "x" || input === "X")
            actions.onToggleAutopilot();
        if (input === "m" || input === "M")
            actions.onRunnersView();
        if (input === "u" || input === "U")
            actions.onAutopilotView();
        if (input === "e" || input === "E")
            actions.onEscalate();
        if (input === ",")
            actions.onSettingsView();
        // Filter (/ key)
        if (input === "/")
            actions.onFilter();
        ...
        if (screen === "run") {
            if (input === "1")
                actions.onDetailTabShortcut(0);
            if (input === "2")
                actions.onDetailTabShortcut(1);
            if (input === "3")
                actions.onDetailTabShortcut(2);
            if (input === "4")
                actions.onDetailTabShortcut(3);
        }
        // G → go to bottom
        if (input === "G")
            actions.onGoBottom();
