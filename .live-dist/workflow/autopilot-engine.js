export class AutopilotEngine {
    ...
    evaluate(candidate) {
        ...
        const excluded = candidate.labels.some(l => this.config.autopilot.exclude_labels.includes(l));
        if (excluded) {
            return {
                action: "skip",
        ...
        const eligible = candidate.labels.some(l => this.config.autopilot.eligible_labels.includes(l));
        if (!eligible) {
            return {
                action: "skip",
        ...
        // Check priority
        const isPriority = candidate.labels.some(l => this.config.autopilot.priority_labels.includes(l));
        if (isPriority) {
        ...
        if (candidateIdx > maxIdx) {
            factors.push(`complexity ${candidate.complexity} exceeds max ${this.config.autopilot.max_complexity}`);
            return {
                action: "escalate",
    ...
    shouldProceedAfterGate(confidence, changedFiles) {
        const factors = [];
        if (confidence < this.config.autopilot.min_gate_confidence) {
            factors.push(`confidence ${confidence} below threshold ${this.config.autopilot.min_gate_confidence}`);
            return {
                action: "escalate",
        ...
        if (changedFiles > this.config.autopilot.max_changed_files) {
            factors.push(`${changedFiles} files changed exceeds max ${this.config.autopilot.max_changed_files}`);
            return {
                action: "escalate",
