...
function createLauncher(config, registry) {
...
export async function triageCommand(args) {
    const issueNumber = parseInt(args[0], 10);
    if (!issueNumber || isNaN(issueNumber)) {
...
export function statusCommand(args) {
    ...
    try {
        ...
        if (run.prUrl)
            console.log(`  PR:       ${run.prUrl}`);
        if (run.repairRound > 0)
            console.log(`  Repairs:  ${run.repairRound} round(s)`);
...
function formatCliDuration(start, end) {
...
export async function reworkCommand(args) {
    const runId = args[0];
    if (!runId) {
...
export async function resolveCommentsCommand(args) {
    const issueNumber = parseInt(args[0], 10);
    if (!issueNumber || isNaN(issueNumber)) {
...
export async function fixCICommand(args) {
    const issueNumber = parseInt(args[0], 10);
    if (!issueNumber || isNaN(issueNumber)) {
...
export function artifactsCommand(args) {
    const runId = args[0];
    if (!runId) {
...
export function listCommand(args = []) {
    ...
    try {
        ...
        // Summary line
        const count = (s) => byStatus.get(s)?.length ?? 0;
        ...
        for (const status of statusOrder) {
            ...
            for (const run of group) {
                ...
                console.log(`    ${run.id.slice(0, 8)}  #${String(run.issueNumber).padEnd(5)} ${run.repo.padEnd(25)} ${age.padStart(6)}${titleStr}`);
            }
        }
        console.log();
    }
...
export function doctorCommand(_args = []) {
    const repoRoot = detectRepoRoot();
    const config = loadWorkflowConfig(repoRoot);
    console.log(`\n  devagent-hub doctor`);
    console.log(`  ${"─".repeat(60)}`);
