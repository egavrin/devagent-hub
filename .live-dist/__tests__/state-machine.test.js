...
import { canTransition, getValidTransitions, assertTransition, getNextPhase, } from "../workflow/state-machine.js";
describe("workflow state machine", () => {
    describe("canTransition", () => {
        it("allows valid forward transitions", () => {
            ...
            expect(canTransition("awaiting_local_verify", "draft_pr_opened")).toBe(true);
            expect(canTransition("draft_pr_opened", "awaiting_human_review")).toBe(true);
            expect(canTransition("awaiting_human_review", "ready_to_merge")).toBe(true);
        ...
        it("allows auto-review fix loop", () => {
            expect(canTransition("draft_pr_opened", "auto_review_fix_loop")).toBe(true);
            expect(canTransition("auto_review_fix_loop", "draft_pr_opened")).toBe(true);
            expect(canTransition("auto_review_fix_loop", "awaiting_human_review")).toBe(true);
        });
    ...
    describe("assertTransition", () => {
        ...
        it("throws on invalid transition", () => {
            expect(() => assertTransition("new", "implementing")).toThrow(/Invalid workflow transition: new -> implementing/);
        });
        it("throws on transition from terminal state", () => {
            expect(() => assertTransition("done", "new")).toThrow(/Invalid workflow transition/);
        });
    });
