...
describe("AutopilotEngine", () => {
    describe("evaluate", () => {
        it("skips excluded labels", () => {
            ...
            const decision = engine.evaluate(makeCandidate({ labels: ["devagent", "blocked"] }));
        ...
        it("skips without eligible labels", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ labels: ["bug"] }));
            expect(decision.action).toBe("skip");
        ...
        it("accepts eligible trivial issues", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ complexity: "trivial" }));
        ...
        it("accepts eligible small issues", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ complexity: "small" }));
            expect(decision.action).toBe("run");
            expect(decision.reason).toBe("Eligible for autopilot");
        });
        it("escalates complex issues (large)", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ complexity: "large" }));
        ...
        it("escalates complex issues (epic)", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ complexity: "epic" }));
            expect(decision.action).toBe("escalate");
            expect(decision.reason).toBe("Complexity too high for autopilot");
        });
        it("detects priority labels", () => {
            const engine = makeEngine();
            const decision = engine.evaluate(makeCandidate({ labels: ["devagent", "priority"], complexity: "small" }));
            expect(decision.action).toBe("run");
