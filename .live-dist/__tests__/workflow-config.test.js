...
import { parseWorkflowConfig, loadWorkflowConfig, defaultConfig, validateConfig, WorkflowConfigError, } from "../workflow/config.js";
...
describe("validateConfig", () => {
    ...
    it("accepts valid modes: assisted, watch, autopilot", () => {
        for (const mode of ["assisted", "watch", "autopilot"]) {
            const cfg = { ...defaultConfig(), mode };
            expect(() => validateConfig(cfg)).not.toThrow();
        }
    });
    it("rejects invalid mode", () => {
        const cfg = { ...defaultConfig(), mode: "yolo" };
        expect(() => validateConfig(cfg)).toThrow(WorkflowConfigError);
    });
