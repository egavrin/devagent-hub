...
function successResponse(phase) {
    return {
        exitCode: 0,
...
async function advanceToImplemented(orchestrator, github, launcher) {
...
describe("WorkflowOrchestrator — verify & PR", () => {
    ...
    it("verify transitions to failed when agent fails", async () => {
        ...
        const failComment = issue.comments.find((c) => c.body.includes("verification failed"));
    ...
    it("opens draft PR after successful verification", async () => {
        ...
        const issue = await github.fetchIssue(REPO, 42);
        const prComment = issue.comments.find((c) => c.body.includes("opened a draft PR"));
    ...
    it("openPR throws if status is not awaiting_local_verify", async () => {
        ...
        await expect(orchestrator.openPR(42)).rejects.toThrow(/expected status "awaiting_local_verify"/);
    });
