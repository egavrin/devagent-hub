...
async function setupPlanAccepted(orchestrator, github, launcher) {
...
describe("WorkflowOrchestrator — implement", () => {
    ...
    it("fails implementation gracefully", async () => {
        ...
        const failComment = updatedIssue.comments.find((c) => c.body.includes("implementation failed"));
    ...
    it("throws if workflow run is not in plan_accepted status", async () => {
        ...
        await orchestrator.triage(42);
        await expect(orchestrator.implement(42)).rejects.toThrow(/expected status "plan_accepted"/);
    });
});
