...
function issueId(projectId, issue) {
    return `${projectId}:issue:${issue.number}`;
}
function mapExecutorId(bin) {
    if (bin?.includes("codex"))
        return "codex";
    if (bin?.includes("claude"))
        return "claude";
    if (bin?.includes("opencode"))
        return "opencode";
    return "devagent";
}
export class WorkflowService {
    ...
    project;
    config;
    constructor(store, github, runnerClient, project, config) {
        this.store = store;
        this.github = github;
    ...
    async syncIssues() {
        const issues = await this.github.fetchEligibleIssues(this.project.repoFullName, this.config.tracker.issue_labels_include);
        return issues.map((issue) => this.upsertIssue(issue));
    }
    async ensureIssue(externalId) {
    ...
    async openPr(workflowId) {
        ...
        if (!latestTask)
            throw new Error("No task attempts found");
        const attempts = this.store.listAttempts(latestTask.id);
        const latestAttempt = attempts[attempts.length - 1];
        if (!latestAttempt?.workspacePath)
            throw new Error("No workspace available for PR handoff");
    ...
    resolveExecutor(stage) {
        ...
        return {
            ...
            reasoning: (profile.reasoning ?? this.config.runner.reasoning),
            approvalMode: (profile.approval_mode ?? this.config.runner.approval_mode),
        };
    }
    async runStage(workflow, workItem, stage) {
