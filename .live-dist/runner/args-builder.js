...
export function validatePhase(phase) {
    if (!VALID_PHASES.has(phase)) {
        throw new InvalidLaunchConfigError(`Invalid workflow phase "${phase}". Valid phases: ${[...VALID_PHASES].join(", ")}`);
    }
}
export function validateApprovalMode(mode) {
    if (!VALID_APPROVAL_MODES.has(mode)) {
        throw new InvalidLaunchConfigError(`Invalid approval mode "${mode}". Valid modes: ${[...VALID_APPROVAL_MODES].join(", ")}`);
    }
}
export function validateReasoningLevel(level) {
    if (!VALID_REASONING_LEVELS.has(level)) {
        throw new InvalidLaunchConfigError(`Invalid reasoning level "${level}". Valid levels: ${[...VALID_REASONING_LEVELS].join(", ")}`);
    }
}
// ─── Arg Builder ─────────────────────────────────────────────
/**
 ...
 */
export function buildLaunchArgs(params, options) {
