...
const describeRunnerCache = new Map();
/**
 ...
 */
export function describeRunner(bin) {
...
export function clearDescribeRunnerCache() {
    describeRunnerCache.clear();
}
/** Convert a RunnerDescription to a full RunnerProtocol. */
export function toRunnerProtocol(desc) {
    return {
 ...
 */
export function validateRunnerCompat(desc) {
    const warnings = [];
    const errors = [];
    if (!desc) {
        return {
    ...
    if (protocol.contractVersion < RUNNER_CONTRACT_VERSION) {
        warnings.push(`Runner contract version ${protocol.contractVersion} is older than Hub expects (${RUNNER_CONTRACT_VERSION}).`);
    }
 ...
 */
export function validateProfileAgainstRunner(profile, desc) {
    const warnings = [];
    if (!desc)
        return ["Cannot validate: runner does not support describe"];
