...
export interface BacklogItem {
  ...
  dependencies: string[]; // titles of items this depends on
}
 ...
 */
export function seedBacklog(brief: ProjectBrief): BacklogItem[] {
  const items: BacklogItem[] = [];

  // Build a map of feature name → milestone name for lookup
  const featureToMilestone = new Map<string, string>();
  ...
  const priorFeatureTitles: string[] = [];
...
function buildBody(
  brief: ProjectBrief,
  feature: { name: string; description: string; priority: string },
...
function buildLabels(milestoneName: string | undefined, priority: string): string[] {
