...
const APPROVABLE: WorkflowStatus[] = ["plan_draft", "plan_revision", "awaiting_human_review", "ready_to_merge"];
const CONTINUABLE: WorkflowStatus[] = [
...
const TERMINAL: WorkflowStatus[] = ["done", "failed", "escalated"];
const REWORKABLE: WorkflowStatus[] = ["plan_draft"];
const ALL_SCREENS: Screen[] = ["dashboard", "run", "approvals", "runners", "autopilot", "settings"];
const ACTION_SCREENS: Screen[] = ["dashboard", "run", "approvals"];

/**
 * Build the full action registry.
 * Accepts execute callbacks that are wired by the App component.
 */
export function buildActions(handlers: Record<string, () => void>): Action[] {
  const h = (id: string) => handlers[id] ?? (() => {});
 ...
 */
export function getSuggestedAction(actions: Action[], ctx: ActionContext): Action | null {
 ...
 */
export function getAvailableActions(actions: Action[], ctx: ActionContext): Action[] {
 ...
 */
export function getPaletteActions(actions: Action[], ctx: ActionContext): Action[] {
